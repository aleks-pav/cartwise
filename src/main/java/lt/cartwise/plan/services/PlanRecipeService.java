...
public class PlanRecipeService {
	...
	public void deletePlanRecipe(UserDetails userDetails, Long id) {
		PlanRecipe recipePlan = planRecipeRepository.findById(id).orElseThrow( () -> new NotFoundException("Plan recipe not found"));
		planService.getPlanOptional(userDetails, recipePlan.getPlan().getId())
			.ifPresentOrElse(p -> planRecipeRepository.delete(recipePlan)
	...
	public PlanRecipeDto putPlanRecipe(UserDetails userDetails, @Valid PlanRecipePatchRequest dto) {
		...
		PlanRecipe recipePlan = planRecipeRepository.findByIdAndPlan_User(dto.id(), user).orElseThrow( () -> new NotFoundException("Plan recipe not found"));
