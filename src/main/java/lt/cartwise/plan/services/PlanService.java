...
public class PlanService {
	...
	public void deletePlan(UserDetails userDetails, Long planId) {
		Plan plan = getPlanOptional(userDetails, planId)
				.orElseThrow(() -> new NotFoundException("Plan with user not found"));
		planRepository.delete(plan);
	}

	@Transactional
	public String createShoppingList(UserDetails userDetails, Long id) {
		Plan plan = getPlanOptional(userDetails, id)
				.orElseThrow(() -> new NotFoundException("Plan with user not found"));
	...
	public Map<Product, Map<Unit, Double>> calculateProducts(Long planId) {
		...
		return recipes.stream().flatMap(planRecipe -> {
			Double ratio = planRecipe.getPortions() / planRecipe.getRecipe().getPortions();
			return planRecipe.getRecipe().getIngidients().stream().map(ing -> {
		...
		}).collect(Collectors.toMap(Ingridient::getProduct, ingridient -> {
	...
	private void deactivateAll(Long userId) {
		List<Plan> plans = planRepository.findByIsActiveAndUserIdOrderByCreatedAtDesc(true, userId).stream()
				.map(plan -> {
