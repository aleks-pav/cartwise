...
public class ShoppingListController {
	...
	public ResponseEntity<ShoppingListDto> getShoppingList(@PathVariable String id) {
		return ResponseEntity.of(shoppingListService.getOptionalDtoById(id));
	}

	@PatchMapping("/{id}")
	public ResponseEntity<ShoppingListProductDto> switchCompleted(@PathVariable String id,
			@RequestBody ShoppingListPatchRequest dto) {
